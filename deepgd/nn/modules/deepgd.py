...
#from ...functions import generate_rand_pos
...
class DeepGD(nn.Module):
    def __init__(self, 
        ...
        # 3d coordinates
        self.in_blocks = nn.ModuleList([
            GNNBlock(feat_dims=[3, 8, 8 if layer_dims is None else layer_dims[0]], bn=True, dp=0.2, static_efeats=2)
        ])
    ...
    def forward(self, data, output_hidden=False, numpy=False):
        ...
        # turn predicted node coordinates into predicted viewpoints
        v = pyg.nn.global_mean_pool(v, data.batch)
