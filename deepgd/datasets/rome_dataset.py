...
class RomeDataset(pyg.data.InMemoryDataset):
    ...
    def convert(self, G):
        ...
        pos_var = self.initializer(G)
        if isinstance(pos_var, dict):
            init_pos = torch.tensor(np.array(list(pos_var.values())))
        else:
            init_pos = torch.tensor(self.initializer(G))

        # fix azimuth between -180 and 180 degrees
        elevation = (torch.rand(1) - 0.5) * 180
        azimuth = (torch.rand(1) - 0.5) * 360
        init_viewpoint = torch.cat((elevation, azimuth))
